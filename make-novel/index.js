...
function doc2vec(num) {
  return new Promise((resolve, reject) => {
    child = exec('python ../../doc2vec/gen.py ' + num,
    ...
    generate(METHOD, num).then(result => {
      ...
      inquirer.prompt([{
        ...
        validate: function( value ) {
          var pass = value.match(/\d/) && Number.parseInt(value) < 10;
          if (pass) {
            return true;
          } else {
        ...
        resolve(result.choices[+answers.answer]);
      });
    });
  }).catch((e) => console.log(e));
}
...
function nextQuestion(num) {
  question(num).then(result => {
    ...
    if (count < total) {
      ...
      console.log(novel);
      var fs = require('fs');
      fs.appendFile(__dirname+'/novel.txt', novel, err => {
          if(err) {
